...
#############################
barcode_stats = {}
for f in glob.glob("ATAC/*/*/*/qc_stats/01_match_barcodes.json"):
...
bowtie2_stats = {}
for f in glob.glob("ATAC/*/*/*/03_atac_bowtie2.log"):
    bowtie2_stats = add_stat_counts(bowtie2_stats, parse_bowtie2_log(f))

overall_alignment_rate = 100 * (bowtie2_stats["concordant_1"] + bowtie2_stats["concordant_multi"] + bowtie2_stats["discordant_1"] + \
        0.5*bowtie2_stats["aligned_mates_1"] + 0.5*bowtie2_stats["aligned_mates_multi"]) / bowtie2_stats["total_reads"]
...
unique_fragments = 0
chrM_fragments = 0
for l in gzip.open("ATAC/fragments.tsv.gz"):
...
json.dump({
    ...
    "high_quality_alignments": total_fragments,
    "unique_fragments": unique_fragments,
    "percent_chrM": chrM_fragments/unique_fragments
}, sys.stdout, indent = 2)
